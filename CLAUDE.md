...
- `data/` — **Not tracked in git** (in `.gitignore`). Contains 33 list definitions and problem JSON files. Imported into Supabase via `build_database.py`.
...
**Tests**: 210 TypeScript (shared 79, worker 45, web 86) + 20 Python (sync script). TS tests: `pnpm exec vitest run` inside each package dir. Python tests: `cd scripts && python3 -m pytest tests/ -v`. CI runs TS tests via `pnpm --filter @caffecode/{shared,worker,web} test`.
