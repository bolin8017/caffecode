...
**Content**: 32 curated lists, 451 problems with AI-generated content. Data files stored locally (not in git), imported via `scripts/build_database.py`.
...
- `data/` — **Not tracked in git** (in `.gitignore`). Contains 32 list definitions and 451 problem JSON files. Imported into Supabase via `build_database.py`.
- `scripts/build_database.py` — Multi-list importer (`--list {slug}`); reads `apps/web/.env.local`
...
**Tests**: 164 total (shared 68, worker 45, web 51). Run: `pnpm exec vitest run` inside each package dir. CI runs all via `pnpm --filter @caffecode/{shared,worker,web} test`.
