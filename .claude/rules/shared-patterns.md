...
- `src/channels/` — `sendTelegramMessage`, `sendLineMessage`, `sendEmailMessage` return `SendResult` with `shouldRetry`. Worker channel classes delegate here; admin `forceNotifyAll` calls directly.
...
`main: "dist/index.js"` in package.json — Railway runtime needs compiled output. Shared must build before worker.
